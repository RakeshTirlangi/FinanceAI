...
PDF.GlobalWorkerOptions.workerSrc = pdfWorker; // Import the worker entry point
...
function HomePage() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (file.name.endsWith('.txt')) {
        ...
        content = Object.keys(workbook.Sheets)
          .map(sheetName => {
      ...
      // Log the content for testing
      //console.log('File content:', content);
      console.log("setfile content",content);
      
      // Optional: Show success message
      alert(`File ${file.name} processed successfully! Content logged to console.`);

    } catch (error) {
      console.error('Error processing file:', error);
      setFileUploadError('Error processing file: ' + (error.message || 'Unknown error'));
  ...
  const analysysDiveData = {
    "section1": {
      "title": "What We Own & What We Owe",
      "question": "What does the bank own, and what does it owe? Is it financially strong?",
      "icon": Warehouse,
      "analysis": [
        "XXX Bank is growing much faster than other banks, lending to risky businesses.",
        "The bank claims very few loans are bad, which seems unlikely.",
        "The bank's safety net for bad loans is smaller than other banks."
      ],
      "meaning": "XXX Bank is taking big risks and may not have enough money to cover potential losses.",
      "severity": "high"
    },
    "section2": {
      "title": "How We Make (or Lose) Money",
      "question": "Is the bank really making money, and is it sustainable?",
      "icon": CircleDollarSign,
      "analysis": [
        "A large part of the bank's income comes from fees and commissions, not just lending.",
        "The bank relies heavily on borrowing money from big institutions.",
        "The bank might be hiding bad loans by giving new loans or changing loan terms."
      ],
      "meaning": "The bank's profits might not be reliable, and it could be hiding problems.",
      "severity": "medium"
    },
    "section3": {
      "title": "Where Our Cash Goes",
      "question": "Does the bank have enough cash, and is it stable?",
      "icon": TrendingIcon,
      "analysis": [
        "The bank depends on borrowed money, making it vulnerable.",
        "The CEO has too much control, and key executives are leaving.",
        "The bank's owners are borrowing money against their shares."
      ],
      "meaning": "The bank is unstable because it relies on borrowed money, lacks oversight, and is losing key people.",
      "severity": "critical"
    },
    "finalTakeaway": "XXX Bank is in a risky situation due to rapid growth, risky lending, and potential hidden problems."
  };
  const getSeverityColor = (severity) => {
    switch(severity) {
      case 'high': return 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20';
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      ...
      {/* Analysis Dive Section */}
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gray-50 dark:bg-gray-900">
        <FinancialNarrativeGenerator financialStatementContent={fileContent}/>
      </section>
